...
class AppointmentViewSet(viewsets.ModelViewSet):
    ...
    def cancel(self, request, pk=None):
        ...
        # Check permission: only the patient or the doctor can cancel
...
class MedicalRecordViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        # Set the doctor to the current user if it's a doctor creating
        user = self.request.user
        if user.role == 'doctor':
            # Ensure the doctor has a Doctor profile
            doctor = get_object_or_404(Doctor, user=user)
            serializer.save(doctor=doctor)
        else:
            # Admin can also create, but we'll keep it simple
            serializer.save()